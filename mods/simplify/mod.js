(() => {
	...
	class Simplify {
		...
		loadScript(url, callback, errorCb) {
			...
			if (callback || errorCb) {
				result.then(callback).catch(errorCb);
			}
		...
		_initializeGameMain() {
			...
			cc.ig.gameMain.getEntityPosition = (entity) => {
				if (!entity || !entity[entries.entityData]) return { x: -1, y: -1, z: -1 };
			...
			cc.ig.gameMain.loadMap = function (data) {
				cc.ig.gameMain[entries.gameMainLoadMap].call(data.context, data);
			};

			cc.ig.TeleportPosition = ig[entries.TeleportPosition];
			cc.ig.TeleportPosition.createFromJson =
				cc.ig.TeleportPosition[entries.TeleportPositionFromJson];
		}
		_initializeCombat() {
			for (const key in sc) {
		...
		_initializeOptions() {
			...
			if (infoBoxSupported) {
				ig.lang.labels.sc.gui.options['mods-description'] = {
					description:
				...
				const descriptionEntry = this.options.addEntry(
					'mods-description',
					'INFO',
					undefined,
					tab,
					'options.mods-description.description'
				);
			...
			for (const mod of mods) {
				...
				const modOption = this.options.addEntry(
					optionName,
					'CHECKBOX',
					true,
					tab,
				...
				const description = infoBoxSupported
					? mod.description || ' '
					: (mod.description || 'If checked this mod is enabled.') +
					  ' \\c[1]Needs a restart!';
	...
	class SimplifyFont {
		...
		_findIconSet() {
			...
			for (const key in font) {
				if (
					typeof font[key] === 'object' &&
					font[key].constructor.name === 'Array' &&
					font[key].length > 0
				) {
	...
	class SimplifyOptions {
		...
		addEntry(name, type, init, cat, data, restart, header) {
			...
			if (restart !== undefined) obj[this.restartName] = restart;
					...
					set: (value) => {
						value
							? localStorage.setItem(
									'logFlags',
									(localStorage.getItem('logFlags') || 3) | 4
							  )
							: localStorage.setItem(
									'logFlags',
									(localStorage.getItem('logFlags') || 3) & 3
							  );
					}
				},
				'logLevel-warn': {
					...
					set: (value) => {
						value
							? localStorage.setItem(
									'logFlags',
									(localStorage.getItem('logFlags') || 3) | 2
							  )
							: localStorage.setItem(
									'logFlags',
									(localStorage.getItem('logFlags') || 3) & 5
							  );
					}
				},
				'logLevel-error': {
					...
					set: (value) => {
						value
							? localStorage.setItem(
									'logFlags',
									(localStorage.getItem('logFlags') || 3) | 1
							  )
							: localStorage.setItem(
									'logFlags',
									(localStorage.getItem('logFlags') || 3) & 6
							  );
					}
				}
			});
		...
		_getVarNames() {
			return new Promise((resolve, reject) => {
				this.restartName = this._getVarNameByType(
					cc.sc.OPTIONS_DEFINITION.language,
					'boolean'
				);
				if (!this.restartName) {
					reject();
				...
				this.initName = this._getVarNameByType(
					cc.sc.OPTIONS_DEFINITION['skip-confirm'],
					'boolean'
				);
				if (!this.initName) {
					reject();
				...
				this.catName = this._getVarNameByType(
					cc.sc.OPTIONS_DEFINITION['show-money'],
					'number'
				);
				if (!this.catName) {
					reject();
				...
				this.dividerName = this._getVarNameByType(
					cc.sc.OPTIONS_DEFINITION['circuit-text-size'],
					'boolean'
				);
				if (!this.dividerName) {
					reject();
			...
			sc.OptionRow.inject({
				...
				init(...args) {
					...
					if (this.option.icon != null) {
						...
						this.nameGui.hook.pos.x +=
							this.iconGui.hook.pos.x + this.iconGui.hook.size.x;
					}
					...
					if (this.option.version != null) {
						...
						this.versionText.setPos(
							this.typeGui.button.hook.size.x + 8,
							lineHook.pos.y
						);
		...
		_loadTabs(tabBox) {
			for (const tabData of this.tabs) {
				tabBox[entries.optionsTabBoxTab][tabData.name] = tabBox[
					entries.optionsTabBoxCreateTabButton
				].call(
					tabBox,
					tabData.name,
					tabBox[entries.optionsTabBoxTabArray].length,
					tabData.cat
				);
			}
