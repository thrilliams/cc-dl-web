...
(() => {
	...
	class SimplifyResources {
		...
		async generatePatch(original, modified) {
			if (original.constructor === String) original = await this.loadJSON(original);

			if (modified.constructor === String) modified = await this.loadJSON(modified);
		...
		_applyAssetOverrides(path) {
			if (!path.startsWith(igroot)) return;
		...
		_hookAjax() {
			$.ajaxSetup({
				beforeSend: async (_, settings) => {
					...
					let successArgs;
					try {
						successArgs = await this._handleAjaxPatching(
							originalUrl,
							this._waitForAjax(settings)
						);
					} catch (e) {
		...
		async _handleAjaxPatching(url, promise) {
			...
			for (const patch of patches) promises.push(this.loadJSON(patch.path));
			...
			for (let i = 0; i < patches.length; i++) {
				if (values[i + 1].status !== 'resolved') {
					console.error(
						`Could not load patch '${patches[i].path}' for '${url}': `,
						values[i + 1].value
					);
					continue;
				}
