	...
	if (a.constructor === Array) {
		...
		for (let i = 0; i < total.length; i++) {
			if (total[i] in a && !(total[i] in b)) {
				...
				change +=
					diffHeuristic(a[total[i]], b[total[i]], settings) * settings.diffMulSameKey;
			}
		}
	...
	if (trueSettings.optimize) {
		for (let i = 1; i < result.length; i++) {
			...
			if (here['type'] == 'EXIT') {
				...
				if (prev['type'] == 'ENTER') {
					// Crush ENTERs
					if (prev['index'].constructor !== Array) prev['index'] = [prev['index']];
					if (here['index'].constructor !== Array) here['index'] = [here['index']];
		...
		for (let i = 0; i < array.length; i++) {
			if (array[i] == 'POPA') {
				...
				let insertion = diffApplyComment(
					{ type: 'ADD_ARRAY_ELEMENT', index: bi, content: photocopy(b[bi]) },
					settings
				);
				...
				if (xd != null) {
					...
					log.push(
						diffApplyComment(
							{ type: 'SET_KEY', index: bi, content: photocopy(b[bi]) },
							settings
						)
					);
				}
		...
		for (let k in a) {
			if (k in b) {
				if (diffHeuristic(a[k], b[k], settings) >= settings.trulyDifferentThreshold) {
					log.push(
						diffApplyComment(
							{ type: 'SET_KEY', index: k, content: photocopy(b[k]) },
							settings
						)
					);
				} else {
